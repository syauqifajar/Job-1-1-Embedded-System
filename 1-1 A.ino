...
uint8_t broadcastAddress[] = {0x3C, 0x71, 0xBF, 0xF1, 0x4B, 0x08};
...
void OnDataSent(const uint8_t *mac_addr, esp_now_send_status_t status) {
 ...
 Serial.println(status == ESP_NOW_SEND_SUCCESS ? "Sukses Terkirim" : "GagalTerkirim");
}
void setup() {
 // Init Serial Monitor
 Serial.begin(115200);
...
void loop() {
 // Set values to send
 strcpy(myData.a, "INI TYAS & SATRIA");
